			...
			<div class="mainContent fl">
				<div class="pnlContent">
					<div class="pnlDestaque"> 
						<p class="categoryTitle"><strong >DESTAQUE</strong></p>
						<hr>
						<?php  						
						...
						WHERE A.IC_DESTAQUE_NOTICIA= 1 ORDER BY A.DT_PUBLICACAO_NOTICIA DESC, A.HR_PUBLICACAO_NOTICIA DESC LIMIT 0,1";
						...
						if($result->num_rows > 0)
						{
							...
							<a href='categoria.php?c=$pk_categoria'><h1>$categoria</h1></a>
							</div>
					...
					<div class="pnlArtigos">
						...
						for($i = 1; $i < 9 ; $i++ )
						{
							...
							,B.CD_CATEGORIA 
							FROM NOTICIA A INNER JOIN CATEGORIA B ON (A.CD_CATEGORIA = B.CD_CATEGORIA) 
							WHERE A.CD_CATEGORIA=$i ORDER BY A.DT_PUBLICACAO_NOTICIA DESC, A.HR_PUBLICACAO_NOTICIA DESC LIMIT 0,1";
							$result = $sqlConn->query($sqlSelect);
							if($result->num_rows > 0)
							{
								while ($recent=$result->fetch_assoc())
